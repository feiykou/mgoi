...
class Product extends Model
{
    ...
    // 获取产品的所有礼物分类
    public function getGiftCate($product){
        if(!isset($product) || empty($product)) return [];
        $resData = $product->giftCate;
        $data = [];
        foreach ($resData as $v){
            $result = $v->pivot;
            array_push($data,$result['gcate_id']);
        }
        return $data;
    }
    ...
    protected static function init(){
        ...
        Product::afterInsert(function ($products){
            ...
            // 处理礼品分类
            if(isset($productData['gift_cate'])){
                $reqData = explode(',',$productData['gift_cate']);
        ...
        Product::beforeUpdate(function ($products){
            ...
            // 处理礼品分类
            if(isset($productData['gift_cate'])){
                $giftData = $productData['gift_cate'];
                $self = new self();
                ...
                $reqData = explode(',',$giftData);
                $detachData = [];
                foreach ($sqlData as $item) {
                    $index = array_search($item,$reqData);
                    if($index){
                        unset($reqData[$index]);
                    }else{
                        array_push($detachData,$item);
                    }
                }
    ...
    public function getProductPropArr($id){
        $_radioAttrRes = db('product_prop')->alias('pp')
            ...
            ->join('spec_param p','pp.prop_id = p.id')
            ->where([
                'pp.product_id' => $id,
                'p.type' => 1
            ])->select();
